...
\textwidth 18cm \hoffset -2.7cm \voffset -2cm
                   
\setlength{\textwidth}{170mm} \setlength{\topmargin}{15mm}
\setlength{\textheight}{240mm} \newcommand{\conack}{Conclusions and
  Acknowledgements} \newcommand{\reftype}{Bibliography}
\setcounter{secnumdepth}{3} \setcounter{tocdepth}{3}

\renewcommand{\topfraction}{1.}  \renewcommand{\bottomfraction}{1.}
\renewcommand{\textfraction}{0.}

\pagestyle{headings} \raggedbottom

\makeatletter \def\descriptionlabel#1{\hspace\labelsep \em #1}
% heading for the \chapter command.
\def\@makechapterhead#1{{ \parindent 0pt\raggedright\LARGE \bf \ifnum
    \c@secnumdepth >\m@ne \@chapapp{} \thechapter.\ \ \fi
    #1\par\nobreak\vskip 20pt }}

% heading for the \chapter* command.
\def\@makeschapterhead#1{{ \parindent 0pt\raggedright\LARGE \bf
    #1\par\nobreak\vskip 20pt}}

% titles for the lists of figures and tables:
\def\listoffigures{
 \section*{List of Figures}
 \@starttoc{lof} } \def\listoftables{
...
{\condbreak{3cm}\vspace{5mm}\leavevmode\def\@captype{figure}}{\par}
\def\condbreak#1{\par\@tempdima=#1 \ifdim\pagetotal>0pt
  \advance\@tempdima by -\pagegoal \advance\@tempdima by \pagetotal
  \ifdim\@tempdima>0pt\newpage\fi\fi}
...
\begin{center}\LARGE
  ...
  {Single Particle Tracking Code Treating Transverse Motion with
    ...
    in a Symplectic Manner} \\
  \vspace*{2mm} {\bf User's Reference Manual}
\end{center}
\begin{center}\em
  F. Schmidt, update by R. De Maria, K.~Sjobak
\end{center}
\begin{center}\large
  {\bf Abstract} \\
\end{center}
...
\cleardoublepage
\tableofcontents
\listoftables \cleardoublepage \pagenumbering{arabic}
...
\subparagraph{Description} If power supply ripple is to be considered
...
\subparagraph{Keyword} RIPP \subparagraph{Number of data lines}
variable

\subparagraph{Format} {\em name ripple--depth ripple--frequency
  start--phase nrturn}
...
\item first data line: {\em partnum emitnx emitny sigz sige ibeco
    ibtyp lhc ibbc} 
\item other data lines: {\em name ibsix xang xplane xtr}
\end{itemize}
...
\item [xstr] (float) Angle of the position of the slices in the boosted frame [rad] (i.e. $X=Z \sin({\rm xstr}) \cos({\rm xplane})$, $Y= \sin({\rm xstr}) \cos({\rm xplane})$ ). In absence of crabbing user should make sure xstr=xang.
\end{description}
...
\subparagraph{Format}
\begin{itemize}
\item data line 1: {\em numl numlr napx amp(1) amp0 ird imc}
\item data line 2: {\em idy(1) idy(2) idfor irew iclo6} \/(integers)
\item data line 3: {\em nde(1) nde(2) nwr(1) nwr(2) nwr(3) nwr(4)
    ntwin ibidu iexact} \/(integers)
\end{itemize}
...
\chapter*{\conack}
\addcontentsline{toc}{chapter}{\conack}
...
\vspace{1em} \scriptsize \centering
\begin{tabular}{|c|c|c|c|c|c|c|}
  \hline \rule[-4mm]{0mm}{10mm} {\bf \#} & {\bf Keyword} &
  \multicolumn{2}{c|}{\bf Input--data--block} & {\bf Short
    Description} & {\bf \S } & {\bf Page} \\
  \hline \rule[-3mm]{0mm}{8mm}
  & & {\bf Title} & {\bf \# of Data--lines} & & & \\
...
\begin{table}[t]
\caption{List of Keywords (continued)}
\vspace{1em} \scriptsize \centering
\begin{tabular}{|c|c|c|c|c|c|c|}
  \hline \rule[-4mm]{0mm}{10mm} {\bf \#} & {\bf Keyword} &
  \multicolumn{2}{c|}{\bf Inputdatablock} & {\bf Short
    Description} & {\bf \S } & {\bf Page} \\
  \hline \rule[-3mm]{0mm}{8mm}
  & & {\bf Title} & {\bf \# of Data--lines} & & & \\
  \hline \stepcounter{kwc} \rule[-1mm]{0mm}{5mm} \thekwc & LINE &
  Linear Optics & 1 & Calculation of all Linear
  Parameters &~\ref{LinOpt} & \pageref{LinOpt} \\
  \rule[-2mm]{0mm}{5mm}
  & & & & of the Accelerator & & \\
  ...
  Power Supply & variable & Invokes a Sinusoidal Tune &~\ref{PowRip} &
  \pageref{PowRip} \\
  \rule[-2mm]{0mm}{5mm}
  & & Ripple & & Variation & & \\
...
The program uses a couple of files for its input and output
procedures.
...
\begin{tabular}{|c|c|c|c|c|}
  \hline \rule[-5mm]{0mm}{12.5mm} {\bf File Unit} & {\bf Input} & {\bf
    Output} & {\bf File Type} & {\bf Contents}
  \\
  \hline
...
\begin{table}[H]
\caption{List of Input and Output Files (continued)}
\vspace{1em} \centering
\begin{tabular}{|c|c|c|c|c|}
  \hline \rule[-5mm]{0mm}{12.5mm} {\bf File Unit} & {\bf Input} & {\bf
    Output} & {\bf File Type} & {\bf Contents}
